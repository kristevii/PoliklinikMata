...
if ($role != 'Dokter' && $role != 'dokter' && 
    ...
    $_SESSION['notif_status'] = 'error';
    $_SESSION['notif_message'] = 'Anda tidak memiliki hak akses untuk menambah data antrian.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_antrian'])) {
    // Ambil nomor antrian dari hidden field (sudah digenerate di frontend)
    ...
    $jenis_antrian = mysqli_real_escape_string($db->koneksi, $_POST['jenis_antrian'] ?? 'Baru');
    $status = mysqli_real_escape_string($db->koneksi, $_POST['status'] ?? 'Menunggu');
    $tanggal_antrian = $_POST['tanggal_antrian'] ?? null;
    
    // Validasi input
    if (empty($nomor_antrian)) {
        $_SESSION['notif_status'] = 'error';
        $_SESSION['notif_message'] = 'Nomor antrian tidak valid!';
        header("Location: ../../dataantrian.php");
        exit();
    }
    ...
    if (empty($id_pasien)) {
        $_SESSION['notif_status'] = 'error';
        $_SESSION['notif_message'] = 'Field pasien wajib diisi!';
        header("Location: ../../dataantrian.php");
        exit();
    }
    ...
    // Cek apakah nomor antrian sudah digunakan hari ini
    $today = date('Y-m-d');
    $check_query = "SELECT id_antrian FROM data_antrian 
                    WHERE nomor_antrian = '$nomor_antrian' 
                    AND DATE(waktu_daftar) = '$today'";
    $check_result = mysqli_query($db->koneksi, $check_query);
    
    if (mysqli_num_rows($check_result) > 0) {
        $_SESSION['notif_status'] = 'error';
        $_SESSION['notif_message'] = "Nomor antrian $nomor_antrian sudah digunakan hari ini!";
        header("Location: ../../dataantrian.php");
        exit();
    }
    
    // Tambah data antrian
    if ($db->tambah_data_antrian($nomor_antrian, $id_pasien, $kode_dokter, $jenis_antrian, $status, $tanggal_antrian_formatted)) {
        // Log aktivitas
        $username_session = $_SESSION['username'] ?? 'unknown user';
        $entitas = 'Tambah';
        $jenis = 'Antrian';
        $keterangan = "Antrian '$nomor_antrian' (Jenis: $jenis_antrian) berhasil ditambahkan oleh $username_session.";
        $waktu = date('Y-m-d H:i:s');
        $db->tambah_aktivitas_user($entitas, $jenis, $keterangan, $waktu);
        
        $_SESSION['notif_status'] = 'success';
        $_SESSION['notif_message'] = "Data antrian $nomor_antrian berhasil ditambahkan.";
    } else {
        $_SESSION['notif_status'] = 'error';
        $_SESSION['notif_message'] = 'Gagal menambahkan data antrian: ' . mysqli_error($db->koneksi);
    }
