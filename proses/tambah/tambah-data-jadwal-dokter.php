...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_jadwal'])) {
    ...
    // Validasi data
    if (empty($kode_dokter) || empty($hari) || empty($shift) || empty($jam_mulai) || empty($jam_selesai)) {
    ...
    try {
        // Tambah data jadwal
        $id_jadwal = $db->tambah_data_jadwal_dokter($kode_dokter, $hari, $shift, $status, $jam_mulai, $jam_selesai);
        ...
        $entitas = 'Tambah';
        $jenis = 'Jadwal Dokter';
        $keterangan = "Jadwal baru (ID: $id_jadwal) berhasil ditambahkan oleh $username_session.";
        
        $_SESSION['notif_status'] = 'success';
        $_SESSION['notif_message'] = 'Data jadwal berhasil ditambahkan.';
        ...
        error_log("Database error saat tambah jadwal: " . $error_detail);
